 ...
 *
 * \param[in] type Asyn type (from asynDriver
...
asynParamType dtypStringToAsynType(char *dtype) {
  ...
  if (strcmp("asynInt8ArrayIn", dtype) == 0 ||
      strcmp("asynInt8ArrayOut", dtype) == 0) {
    return asynParamInt8Array;
  }
  if (strcmp("asynInt16ArrayIn", dtype) == 0 ||
      strcmp("asynInt16ArrayOut", dtype) == 0) {
    return asynParamInt16Array;
  }
  if (strcmp("asynInt32ArrayIn", dtype) == 0 ||
      strcmp("asynInt32ArrayOut", dtype) == 0) {
    return asynParamInt32Array;
  }
  if (strcmp("asynFloat32ArrayIn", dtype) == 0 ||
      strcmp("asynFloat32ArrayOut", dtype) == 0) {
    return asynParamFloat32Array;
  }
  if (strcmp("asynFloat64ArrayIn", dtype) == 0 ||
...
int windowsToEpicsTimeStamp(uint64_t plcTime, epicsTimeStamp *ts) {
  ...
  // move from WindowsTime to Epics time 1601 jan 1 to 1990 jan 1
  // (POSIX_TIME_AT_EPICS_EPOCH defined in epicsTime.h)
  plcTime = plcTime - (POSIX_TIME_AT_EPICS_EPOCH + SEC_TO_UNIX_EPOCH) *
                          WINDOWS_TICK_PER_SEC;

  ts->secPastEpoch = (uint32_t)(plcTime / WINDOWS_TICK_PER_SEC);
  ts->nsec =
      (uint32_t)((plcTime - (ts->secPastEpoch * WINDOWS_TICK_PER_SEC)) * 100);
 ...
 * \return 0 or error code.
 */
int addToBuffer(adsOctetOutputBufferType *buffer, const char *addText,
                size_t addLength) {
  if (buffer == NULL) {
    return __LINE__;
  }
 ...
 * \return 0 or error code.
 */
static int cmd_buf_vprintf(adsOctetOutputBufferType *buffer, const char *format,
                           va_list arg) {
 ...
 * \return 0 or error code.
 */
int octetCreateArgvSepv(const char *line, const char ***argv_p,
                        char ***sepv_p) {
...
int octetBinary2ascii(bool returnVarName, void *binaryBuffer,
  ...
  while (bytesProcessed < info->size && !error) {
    ...
    case ADST_BIT:
      ...
      bytesPerDataPoint = 1; // TODO: Check if each bit takes one byte or
                             // actually only one bit?!
      bytesProcessed += bytesPerDataPoint;
      break;
    case ADST_STRING:
      RETURN_VAR_NAME_IF_NEEDED;
