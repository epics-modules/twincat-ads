 ...
 * \return void
 * Will be called be the EPICS framework with the current EPICS state as it
 * changes.
 */
 ...
 * \return void
 */
int initHook(void) { return (initHookRegister(getEpicsState)); }

/** Callback from ads lib for symbols changed in PLC.
 * \param[in] pAddr AmsAddr of the system generating the callback.
 * \param[in] pNotification Data structure containing the updated data and
 * timestamp information. \param[in] hUser Identification index of the callback
 * parameter. \return void This function will be called by the ADS lib if the
 * symbol version in the PLC is changed.
 */
static void
 ...
 * \param[in] pNotification Data structure containing the updated data and
 * timestamp information. \param[in] hUser Identification index of the callback
 * parameter. \return void This function will be called by the ADS lib when a
 * registered parameter is updated (changed in PLC).
 */
static void adsDataCallback(const AmsAddr *pAddr,
  ...
  long secs_used =
      (newTime.tv_sec - oldTime.tv_sec); // avoid overflow by subtracting first
  long micros_used =
      ((secs_used * 1000000) + newTime.tv_usec) - (oldTime.tv_usec);
  oldTime = newTime;
  ...
  asynPrint(asynTraceUser, ASYN_TRACEIO_DRIVER,
            "Callback for parameter %s (%d).\n", paramInfo->drvInfo,
            paramInfo->paramIndex);
  asynPrint(asynTraceUser, ASYN_TRACEIO_DRIVER,
            "hUser 0x%x, data size[b]: %d.\n", hUser,
            pNotification->cbSampleSize);
  asynPrint(asynTraceUser, ASYN_TRACEIO_DRIVER,
            "time stamp [100ns]: %" PRIuMAX
            ", since last plc [ms]: %4.2lf, since last ioc [ms]: %4.2lf.\n",
 ...
 *            sent to client (linux). The variable can also be sent sooner if
 the
 ...
 */
adsAsynPortDriver::adsAsynPortDriver(const char *portName, const char *ipaddr,
                                     const char *amsaddr, unsigned int amsport,
                                     int paramTableSize, unsigned int priority,
                                     int autoConnect, int defaultSampleTimeMS,
                                     int maxDelayTimeMS, int adsTimeoutMS,
          ...
          0)             /* Default stack size*/
{
  ...
  int nvals =
      sscanf(amsaddr_, "%hhu.%hhu.%hhu.%hhu.%hhu.%hhu", &remoteNetId_.b[0],
             &remoteNetId_.b[1], &remoteNetId_.b[2], &remoteNetId_.b[3],
             &remoteNetId_.b[4], &remoteNetId_.b[5]);
  if (nvals != 6) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
  ...
  if (paramTableSize_ <
      1) { // If paramTableSize_==1 then only stream device or motor record can
           // use the driver through the "default access" param below.
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
              "%s:%s: Param table size to small: %d\n", driverName,
              functionName, paramTableSize_);
    return;
  }
  ...
  int index;
  asynStatus status =
      createParam("Default access", asynParamNotDefined, &index);
  ...
  status = (asynStatus)(epicsThreadCreate(
                            "adsAsynPortDriverBulkReadThread",
                            epicsThreadPriorityMedium,
                            epicsThreadGetStackSize(epicsThreadStackMedium),
  ...
  while (1) {
    ...
    if (connectedAds_) {
      for (amsPortInfo *port : amsPortList_) {
        long error = 0;
        asynStatus stat =
            adsReadStateLock(port->amsPort, &adsState, true, &error);
    ...
    for (amsPortInfo *port : amsPortList_) {
      if (port->connectedOld != port->connected) {
        asynPrint(pasynUserSelf, ASYN_TRACE_INFO,
                  "%s:%s: Device \"%s\" %s (Ams-port %u, Ams router version "
                  "%u.%u.%u).\n",
                  driverName, functionName, port->devName,
                  port->connected ? "connected" : "disconnected", port->amsPort,
                  port->version.version, port->version.revision,
                  port->version.build);
      }
    ...
    if (!oneAmsConnectionOK) {
      ...
      if (notConnectedCounter_ < 100 || notConnectedCounter_ % 100 == 0) {
        asynPrint(pasynUserSelf, ASYN_TRACE_FLOW,
                  ...
                  functionName, notConnectedCounter_);
      }
    }
    if (oneAmsConnectionOK) {
      notConnectedCounter_ = 0;
  ...
  while (1) {
    ...
    for (int i = 0; bulk[i].cnt; i++) {
      ...
      status = AdsSyncReadWriteReqEx2(
          adsPort_, &amsServer, ADSIGRP_SUMUP_READ, cnt, readSize, bulkdata,
          sizeof(bulk[i].sum[0]) * cnt, &bulk[i].sum, &bytesRead);
      if (status) {
      ...
      for (uint32_t j = 2; j < cnt; j++) {
        ...
        if (*stat++) {
          asynPrint(asynTraceUser, ASYN_TRACE_ERROR,
                    ...
                    functionName, paramInfo->drvInfo, j);
          continue;
        }
...
void adsAsynPortDriver::report(FILE *fp, int details) {
  ...
  if (details >= 1) {
    ...
    fprintf(fp, "  Default time source:         %s\n",
            (defaultTimeSource_ == ADS_TIME_BASE_PLC)
                ? ADS_OPTION_TIMEBASE_PLC
                : ADS_OPTION_TIMEBASE_EPICS);
    fprintf(fp,
            "  NOTE: Several records can be linked to the same parameter.\n");
    fprintf(fp, "\n");
  }
  if (details >= 2) {
    ...
    for (int i = 0; i < adsParamArrayCount_; i++) {
      ...
      if (i == 0) {
        fprintf(fp,
                "    Parameter 0 (pasynUser->reason==0) is reserved for Asyn "
                "octet interface (Motor Record and Stream Device access).\n");
        fprintf(fp, "\n");
        continue;
      }
 ...
 * \return asynSuccess or asynError.
 */
asynStatus adsAsynPortDriver::refreshParams() { return refreshParams(0); }
...
asynStatus adsAsynPortDriver::refreshParams(uint16_t amsPort) {
  ...
  if (connectedAds_) {
    if (adsParamArrayCount_ > 1) {
      ...
      for (int i = 1; i < adsParamArrayCount_;
           i++) { // Skip first param since used for motorrecord or stream
                  // device
        if (!pAdsParamArray_[i]) {
          continue;
        }
...
asynStatus adsAsynPortDriver::invalidateParams(uint16_t amsPort) {
  ...
  if (adsParamArrayCount_ > 1) {
    for (int i = 1; i < adsParamArrayCount_;
         i++) { // Skip first param since used for motorrecord or stream device
      if (!pAdsParamArray_[i]) {
        continue;
      }
      adsParamInfo *paramInfo = pAdsParamArray_[i];
  ...
  for (int i = 0; i < MAXBULK; i++) {
    ...
    if (amsPort == 0 || bulk[i].amsPort == amsPort)
      bulk[i].readSize =
          4 * sizeof(uint32_t); // Initialize to just the timestamp!
  }
  return asynSuccess;
}
 ...
 * This function is called by the asyn-framework for each record that is linked
 * to this asyn port. \param[in] pasynUser Pointer to asyn user structure
 * \param[in] drvInfo String containing information about the parameter.
...
asynStatus adsAsynPortDriver::drvUserCreate(asynUser *pasynUser,
  ...
  if (status == asynSuccess) {
    asynPrint(pasynUser, ASYN_TRACE_FLOW,
              ...
              functionName, index, drvInfo);
    if (!pAdsParamArray_[index]) {
      asynPrint(pasynUser, ASYN_TRACE_ERROR,
                "%s:%s:pAdsParamArray_[%d]==NULL (drvInfo=%s).", driverName,
                functionName, index, drvInfo);
      return asynError;
    }

    if (pAdsParamArray_[index]->dataSource ==
        ADS_DATASOURCE_AMS_STATE) { // Local variable (not in PLC) like AMS port
                                    // state.

      return asynPortDriver::drvUserCreate(pasynUser, drvInfo, pptypeName,
                                           psize);
    }
  ...
  if (adsParamArrayCount_ >= (paramTableSize_ - 1)) {
    asynPrint(pasynUser, ASYN_TRACE_ERROR,
              "%s:%s: Parameter table full. Parameter with drvInfo %s will be "
              "discarded.",
  ...
  if (status != asynSuccess) {
    asynPrint(pasynUser, ASYN_TRACE_ERROR, "%s:%s: createParam() failed.",
              driverName, functionName);
    return asynError;
  }
  asynPrint(pasynUser, ASYN_TRACE_INFO,
            "%s:%s: Parameter created: \"%s\" (index %d).\n", driverName,
            functionName, drvInfo, index);
  ...
  if (connectedAds_ &&
      !(paramInfo->dataSource ==
        ADS_DATASOURCE_AMS_STATE)) { // Do not read info from PLC if local
                                     // variable (like ams-port state)
  ...
  return asynPortDriver::drvUserCreate(pasynUser, drvInfo, pptypeName,
                                       psize); // Assigns pasynUser->reason;
}
 ...
 * \return asynSuccess or asynError.
 * If the PLC variable is an array then a buffer is allocated in the paramInfo
...
adsAsynPortDriver::updateParamInfoWithPLCInfo(adsParamInfo *paramInfo) {
  ...
  // Do not read information from PLC if "variable" in driver (like ams router
  // state)
  ...
  if (isArray) {
    if (paramInfo->plcSize != paramInfo->arrayDataBufferSize &&
        paramInfo->arrayDataBuffer) { // new size of array
  ...
  for (i = 0; bulk[i].cnt && i < MAXBULK; i++) {
    ...
    if (!name) {
      printf("    0: MAIN.fbSystemTime.timeLoDW (G=0x%x, O=0x%x, S=%d)\n",
             bulk[i].sum[0].iGroup, bulk[i].sum[0].iOffset,
             bulk[i].sum[0].iSize);
      printf("    1: MAIN.fbSystemTime.timeHiDW (G=0x%x, O=0x%x, S=%d)\n",
             bulk[i].sum[1].iGroup, bulk[i].sum[1].iOffset,
             bulk[i].sum[1].iSize);
    }
    for (int j = 2; j < bulk[i].cnt; j++) {
      ...
      if (!name || strstr(paramInfo->plcAdrStr, name))
        printf("  %3d: %s (G=0x%x, O=0x%x, S=%d, TS=%d.%09d)\n", j,
               paramInfo->plcAdrStr, bulk[i].sum[j].iGroup,
               bulk[i].sum[j].iOffset, bulk[i].sum[j].iSize,
               paramInfo->epicsTimestamp.secPastEpoch,
               paramInfo->epicsTimestamp.nsec);
    }
  }
...
asynStatus adsAsynPortDriver::adsAddToBulkRead(adsParamInfo *paramInfo) {
  ...
  bulk[paramInfo->bulkIndex].sum[paramInfo->bulkOffset].iSize =
      paramInfo->plcSize;
...
#define TSHI "MAIN.fbSystemTime.timeHiDW"
  if (bulkTS[i].refreshNeeded) {
    ...
    statH = AdsSyncReadWriteReqEx2(adsPort_, &amsServer, ADSIGRP_SYM_HNDBYNAME,
                                   0, sizeof(uint32_t), &bulkTS[i].iHandleH,
                                   strlen(TSHI), TSHI, nullptr);
    statL = AdsSyncReadWriteReqEx2(adsPort_, &amsServer, ADSIGRP_SYM_HNDBYNAME,
                                   0, sizeof(uint32_t), &bulkTS[i].iHandleL,
                                   strlen(TSLO), TSLO, nullptr);
    if (!statH && !statL)
    ...
    while (!status) {
      ...
      if (!dbIsAlias(pdbentry)) {
        ...
        if (!status) {
          ...
          int nvals = sscanf(paramInfo->inp, "@asyn(%[^,],%d,%d)%s", port, &adr,
                             &timeout, currdrvInfo);
          if (nvals == 4) {
            // Ensure correct port and drvinfo
            if (strcmp(port, portName) == 0 &&
        ...
        if (!status) {
          ...
          int timeout;
          char currdrvInfo[ADS_MAX_FIELD_CHAR_LENGTH];
          int nvals = sscanf(paramInfo->out, "@asyn(%[^,],%d,%d)%s", port, &adr,
                             &timeout, currdrvInfo);
          if (nvals == 4) {
            // Ensure correct port and drvinfo
            if (strcmp(port, portName) == 0 &&
        ...
        if (recordFound) {
          ...
          dbFreeEntry(pdbentry);
          return asynSuccess; // The correct record was found and the paramInfo
                              // structure is filled
        } else {
...
asynStatus adsAsynPortDriver::parsePlcInfofromDrvInfo(const char *drvInfo,
  ...
  asynPrint(pasynUserSelf, ASYN_TRACE_FLOW, "%s:%s: drvInfo %s is %s\n",
            driverName, functionName, drvInfo,
            paramInfo->hasInput ? "has Input (end with ?)"
                                : "output (end with =)");
  ...
  if (drvInfoEnd) { // found '/'
    ...
    if (nvals == 1) {
      ...
      paramInfo->plcAdrStr[strlen(paramInfo->plcAdrStr) - 1] =
          0; // Strip ? or = from end
    } else {
      asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
  ...
  if (isThere) {
    if (strlen(isThere) < (strlen(option) + strlen("16#%x,16#%x,%u,%u"))) {
      asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
                "%s:%s: Failed to parse %s command from drvInfo (%s). String "
                "to short.\n",
    ...
    int nvals = sscanf(isThere + strlen(option), "16#%x,16#%x,%u,%u",
                       &paramInfo->plcAbsAdrGroup, &paramInfo->plcAbsAdrOffset,
                       &paramInfo->plcSize, &paramInfo->plcDataType);
    ...
    if (nvals == 4) {
      ...
      asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
                "%s:%s: Failed to parse %s command from drvInfo (%s). Wrong "
                "format.\n",
                driverName, functionName, option, drvInfo);
      return asynError;
    }
  ...
  if (isThere) {
    if (strlen(isThere) < (strlen(option) + strlen("=0/"))) {
      asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
                "%s:%s: Failed to parse %s option from drvInfo (%s). String to "
                "short.\n",
                driverName, functionName, option, drvInfo);
      return asynError;
    }
    ...
    int nvals =
        sscanf(isThere + strlen(option), "=%lf/", &paramInfo->maxDelayTimeMS);
  ...
  if (isThere) {
    ...
    int nvals =
        sscanf(isThere + strlen(option), "=%lf/", &paramInfo->sampleTimeMS);
  ...
  if (isThere) {
    ...
    int nvals =
        sscanf(isThere + strlen(option), "=%lf/", &paramInfo->pollClass);
  ...
  if (isThere) {
    ...
    if (strlen(isThere) <
        (strlen(option) + strlen("=/") + minLen)) { // Allowed "PLC" or "EPICS"
      asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
                "%s:%s: Failed to parse %s option from drvInfo (%s). String to "
                "short.\n",
                driverName, functionName, option, drvInfo);
  ...
  if (isThere) {
    ...
    paramInfo->dataSource =
        ADS_DATASOURCE_AMS_STATE; // This information is accessible in driver
                                  // (not PLC)
  ...
  return addNewAmsPortToList(
      paramInfo->amsPort); // Only add if not already there
}
...
asynStatus adsAsynPortDriver::addNewAmsPortToList(uint16_t amsPort) {
  ...
  try {
    ...
    newPort->adsState =
        (ADSSTATE)(ADSSTATE_MAXSTATES + 1); // Set unknown state..
...
asynStatus adsAsynPortDriver::readOctet(asynUser *pasynUser, char *value,
  ...
  *nActual = thisRead;
  asynPrint(pasynUser, ASYN_TRACE_FLOW, "%s thisRead=%lu data=\"%s\"\n",
            portName, (unsigned long)thisRead, value);
  unlock();
  asynPrint(pasynUser, ASYN_TRACE_FLOW, "%s:%s:%s\n", driverName, functionName,
            value);
...
asynStatus adsAsynPortDriver::writeOctet(asynUser *pasynUser, const char *value,
  ...
  asynPrintIO(pasynUser, ASYN_TRACEIO_DRIVER, value, maxChars, "%s write %lu\n",
              portName, (unsigned long)maxChars);
  *nActual = 0;
  ...
  if (errorCode) {
    /*Return asyn error if communication is down (all client errors) otherwise
     * asynSuccess but error message in buffer*/
  ...
  *nActual = thisWrite;
  ...
  // unlock();
  asynPrint(pasynUser, ASYN_TRACE_FLOW, "%s wrote %lu return %s.\n", portName,
            (unsigned long)*nActual, pasynManager->strStatus(status));
  return status;
}
...
int adsAsynPortDriver::octetCMDwriteIt(const char *inbuf, size_t inlen) {
  ...
  octetCmdBuf_printf(&octetAsciiBuffer_, "%s%s", had_cr ? "\r" : "",
                     had_lf ? "\n" : "");
...
int adsAsynPortDriver::octetCmdHandleInputLine(
  ...
  char **my_sepv = NULL;
  int argc = octetCreateArgvSepv(input_line, (const char ***)&my_argv,
                                 (char ***)&my_sepv);

  int errorCodeLatch = 0;
  for (int i = 1; i <= argc; i++) {
    int errorCode = octetMotorHandleOneArg(
        my_argv[i], buffer); // Continue with next cmd even if error
    if (errorCode &&
        !errorCodeLatch) { // latch first error code for stacked commands
...
int adsAsynPortDriver::octetMotorHandleOneArg(
  ...
  uint16_t amsPort = amsportDefault_; // should actually be called amsport ( 851
                                      // for first plc as default) ...
...
int adsAsynPortDriver::octetMotorHandleADRCmd(
  ...
  int nvals;
  nvals = sscanf(arg, ".ADR.16#%x,16#%x,%u,%u=", &group_no, &offset_in_group,
                 &len_in_PLC, &type_in_PLC);

  asynPrint(pasynUserSelf, ASYN_TRACE_FLOW,
            "%s:%s: nvals=%d amsport=%u group_no=0x%x offset_in_group=0x%x "
            "len_in_PLC=%u type_in_PLC=%u\n",
            driverName, functionName, nvals, amsport, group_no, offset_in_group,
            len_in_PLC, type_in_PLC);
  ...
  if (myarg_1) {
    ...
    int error = octetAdsWriteByGroupOffset(
        amsport, (uint32_t)group_no, (uint32_t)offset_in_group,
        (uint16_t)type_in_PLC, (uint32_t)len_in_PLC, myarg_1, buffer);
    if (error) {
...
int adsAsynPortDriver::octetAdsReadByName(uint16_t amsPort,
  ...
  long errorCode = 0;
  asynStatus stat =
      adsGetSymInfoByName(amsPort, variableAddr, &infoStruct, &errorCode);
  if (stat != asynSuccess) {
    return errorCode;
  }
...
int adsAsynPortDriver::octetAdsWriteByName(
  ...
  return octetAdsWriteByGroupOffset(
      amsPort, infoStruct.iGroup, infoStruct.iOffset, infoStruct.dataType,
      infoStruct.size, asciiValueToWrite, outBuffer);
}
...
int adsAsynPortDriver::octetAdsReadByGroupOffset(
  ...
  asynPrint(pasynUserSelf, ASYN_TRACE_FLOW,
            ...
            "dataSize: %d.\n",
            driverName, functionName, (int)amsPort, (int)info->iGroup,
            (int)info->iOffset, adsTypeToString(info->dataType),
            (int)info->dataType, (int)info->size);
  ...
  int error =
      AdsSyncReadReqEx2(adsPort_, &amsServer, info->iGroup, info->iOffset,
                        dataSize, &octetBinaryBuffer_, &bytesRead);

  if (error) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
              "%s:%s: ADS read failed with: %s (0x%x).\n", driverName,
              functionName, adsErrorToString(error), error);
    adsUnlock();
    return error;
  }

  error = octetBinary2ascii(octetReturnVarName_, &octetBinaryBuffer_,
                            ADS_CMD_BUFFER_SIZE, info, outBuffer);
  if (error) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
 ...
 */
int adsAsynPortDriver::octetAdsWriteByGroupOffset(
  ...
  asynPrint(pasynUserSelf, ASYN_TRACE_FLOW,
            "%s:%s: amsPort: %d, group: %d, offset: %d, dataType: %s (%d), "
            "dataSize: %d.\n",
            driverName, functionName, (int)amsPort, (int)group, (int)offset,
            adsTypeToString(dataType), (int)dataType, (int)dataSize);
  ...
  int error =
      octetAscii2binary(asciiValueToWrite, dataType, &octetBinaryBuffer_,
                        ADS_CMD_BUFFER_SIZE, &bytesToWrite);
  if (error) {
    adsUnlock();
  ...
  error = AdsSyncWriteReqEx(adsPort_, &amsServer, group, offset, bytesToWrite,
                            &octetBinaryBuffer_);
...
asynStatus adsAsynPortDriver::writeInt32(asynUser *pasynUser,
  ...
  default:
    asynPrint(pasynUser, ASYN_TRACE_ERROR,
              "%s:%s: Data types not compatible (epicsInt32 and %s). Write "
              "canceled.\n",
              driverName, functionName,
              adsTypeToString(paramInfo->plcDataType));
    return asynError;
 ...
 * \return asynSuccess or asynError.
 */
asynStatus adsAsynPortDriver::adsGenericArrayRead(asynUser *pasynUser,
                                                  long allowedType,
  ...
  if (!pAdsParamArray_[paramIndex] || paramIndex >= paramTableSize_) {
    asynPrint(pasynUser, ASYN_TRACE_ERROR,
              "%s:%s: pAdsParamArray NULL or index (pasynUser->reason) our of "
              "range\n",
              driverName, functionName);
 ...
 * \return asynSuccess or asynError.
 */
asynStatus adsAsynPortDriver::adsGenericArrayWrite(asynUser *pasynUser,
                                                   long allowedType,
                                                   const void *data,
                                                   size_t nEpicsBufferBytes) {
...
asynStatus adsAsynPortDriver::readInt8Array(asynUser *pasynUser,
  ...
  asynStatus stat =
      adsGenericArrayRead(pasynUser, allowedType, (void *)value,
                          nElements * sizeof(epicsInt8), &nBytesRead);
  if (stat != asynSuccess) {
    return asynError;
  }
...
asynStatus adsAsynPortDriver::readInt16Array(asynUser *pasynUser,
  ...
  asynStatus stat =
      adsGenericArrayRead(pasynUser, allowedType, (void *)value,
                          nElements * sizeof(epicsInt16), &nBytesRead);
  if (stat != asynSuccess) {
    return asynError;
  }
...
asynStatus adsAsynPortDriver::readInt32Array(asynUser *pasynUser,
  ...
  asynStatus stat =
      adsGenericArrayRead(pasynUser, allowedType, (void *)value,
                          nElements * sizeof(epicsInt32), &nBytesRead);
  if (stat != asynSuccess) {
    return asynError;
  }
...
asynStatus adsAsynPortDriver::readFloat32Array(asynUser *pasynUser,
  ...
  asynStatus stat =
      adsGenericArrayRead(pasynUser, allowedType, (void *)value,
                          nElements * sizeof(epicsFloat32), &nBytesRead);
  if (stat != asynSuccess) {
    return asynError;
  }
...
asynStatus adsAsynPortDriver::readFloat64Array(asynUser *pasynUser,
  ...
  asynStatus stat =
      adsGenericArrayRead(pasynUser, allowedType, (void *)value,
                          nElements * sizeof(epicsFloat64), &nBytesRead);
  if (stat != asynSuccess) {
    return asynError;
  }
...
asynStatus adsAsynPortDriver::adsGetSymHandleByName(adsParamInfo *paramInfo,
  ...
  const long handleStatus = AdsSyncReadWriteReqEx2(
      adsPort_, &amsServer, ADSIGRP_SYM_HNDBYNAME, 0,
      sizeof(paramInfo->hSymbolicHandle), &symbolHandle,
      strlen(paramInfo->plcAdrStr), paramInfo->plcAdrStr, nullptr);
  adsUnlock();
  if (handleStatus) {
    if (!blockErrorMsg) {
      asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
                ...
                adsErrorToString(handleStatus), handleStatus);
    }
    return asynError;
  }
...
asynStatus adsAsynPortDriver::adsAddSymbolsChangedCallback(amsPortInfo *port) {
  ...
  uint32_t hNotify = 0;
  adsLock();
  long addStatus = AdsSyncAddDeviceNotificationReqEx(
      adsPort_, &amsServer, ADSIGRP_SYM_VERSION, 0, &attrib,
...
asynStatus adsAsynPortDriver::adsDelSymbolsChangedCallback(amsPortInfo *port) {
  ...
  const long delStatus = AdsSyncDelDeviceNotificationReqEx(
      adsPort_, &amsServer, port->hCallbackNotify);
  adsUnlock();
...
asynStatus adsAsynPortDriver::adsAddDataCallback(adsParamInfo *paramInfo) {
  ...
  if (paramInfo->isAdrCommand) { // Abs access (ADR command)
    ...
    group = ADSIGRP_SYM_VALBYHND; // Access via symbolic handle stored in
                                  // paramInfo->hSymbolicHandle
    offset = paramInfo->hSymbolicHandle;
  }
  ...
  adsLock();
  long addStatus = AdsSyncAddDeviceNotificationReqEx(
      adsPort_, &amsServer, group, offset, &attrib, &adsDataCallback,
      (uint32_t)paramInfo->paramIndex, &hNotify);
  adsUnlock();
  if (addStatus) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
              "%s:%s: Add device notification failed with: %s (0x%lx)\n",
 ...
 * \return asynSuccess or asynError.
 */
asynStatus adsAsynPortDriver::adsGetSymInfoByName(uint16_t amsPort,
                                                  const char *varName,
                                                  adsSymbolEntry *info) {
  long errorCode = 0;
...
asynStatus adsAsynPortDriver::adsGetSymInfoByName(uint16_t amsPort,
  ...
  adsLock();
  const long infoStatus = AdsSyncReadWriteReqEx2(
      adsPort_, &amsServer, ADSIGRP_SYM_INFOBYNAMEEX, 0, sizeof(adsSymbolEntry),
      info, strlen(varName), varName, &bytesRead);
  adsUnlock();
  *errorCode = infoStatus;
  ...
  if (info->nameLength >= sizeof(info->buffer) - 1) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
              "%s:%s: Missalignment of type in AdsSyncReadWriteReqEx2 return "
              "struct for %s\n",
  ...
  if (info->nameLength + info->typeLength + 2 >=
      (uint16_t)(sizeof(info->buffer) - 1)) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
              "%s:%s: Missalignment of comment in AdsSyncReadWriteReqEx2 "
              "return struct for %s\n",
              driverName, functionName, varName);
  }
...
asynStatus adsAsynPortDriver::adsGetSymInfoByName(adsParamInfo *paramInfo) {
  ...
  asynStatus stat = adsGetSymInfoByName(paramInfo->amsPort,
                                        paramInfo->plcAdrStr, &infoStruct);
  if (stat) {
    return asynError;
  }
...
asynStatus adsAsynPortDriver::adsConnect() {
  ...
  asynPrint(pasynUserSelf, ASYN_TRACE_FLOW,
            ...
            functionName, defaultTimeout, (uint32_t)adsTimeoutMS_);
...
asynStatus adsAsynPortDriver::adsReadVersion(amsPortInfo *port) {
  ...
  adsLock();
  long status =
      AdsSyncReadDeviceInfoReqEx(adsPort_, &amsServer, devName, &version);
  adsUnlock();
  if (status) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
...
asynStatus adsAsynPortDriver::adsDisconnect() {
  ...
  if (closeStatus) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
              "%s:%s: Close ADS port failed with: %s (0x%lx)\n", driverName,
              functionName, adsErrorToString(closeStatus), closeStatus);
    return asynError;
  }
 ...
 */
asynStatus
adsAsynPortDriver::adsReleaseSymbolicHandle(adsParamInfo *paramInfo) {
...
asynStatus adsAsynPortDriver::adsReleaseSymbolicHandle(adsParamInfo *paramInfo,
  ...
  if (paramInfo->bSymbolicHandleValid) {
    ...
    adsLock();
    const long releaseStatus = AdsSyncWriteReqEx(
...
asynStatus adsAsynPortDriver::adsWriteParam(adsParamInfo *paramInfo,
  ...
  adsLock();
  long writeStatus = AdsSyncWriteReqEx(adsPort_, &amsServer, group, offset,
                                       paramInfo->plcSize, binaryBuffer);
  adsUnlock();
  if (writeStatus) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
              "%s:%s: ADS write failed with: %s (0x%lx)\n", driverName,
              functionName, adsErrorToString(writeStatus), writeStatus);
    return asynError;
  }
...
asynStatus adsAsynPortDriver::adsReadParam(adsParamInfo *paramInfo, long *error,
  ...
  *error = 0;
  ...
  uint32_t bytesRead = 0;
  adsLock();
  *error = AdsSyncReadReqEx2(adsPort_, &amsServer, group, offset,
                             paramInfo->plcSize, (void *)data, &bytesRead);
  adsUnlock();
  if (*error) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
 ...
 * \return asynSuccess or asynError.
 */
asynStatus adsAsynPortDriver::adsReadStateLock(uint16_t amsport,
                                               uint16_t *adsState,
                                               bool blockErrorMsg) {
  asynStatus stat;
  lock();
...
asynStatus adsAsynPortDriver::adsReadState(uint16_t amsport, uint16_t *adsState,
  ...
  adsLock();
  const long status =
      AdsSyncReadStateReqEx(adsPort_, &amsServer, adsState, &devState);
  *error = status;
  adsUnlock();
  if (status) {
    if (!blockErrorMsg) {
      asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
                ...
                functionName, adsErrorToString(status), status);
    }
    return asynError;
  }
...
asynStatus adsAsynPortDriver::adsWriteState(uint16_t amsport,
  ...
  AmsAddr amsServer = {remoteNetId_, amsport};
  const long status =
      AdsSyncWriteControlReqEx(adsPort_, &amsServer, adsState, 0, 0, pData);
  if (status) {
    asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
 ...
 */
int adsAsynPortDriver::getParamTableSize() { return paramTableSize_; }
 ...
 */
int adsAsynPortDriver::getAdsParamCount() { return adsParamArrayCount_; }
...
asynStatus adsAsynPortDriver::refreshParamTime(adsParamInfo *paramInfo) {
  ...
  if (windowsToEpicsTimeStamp(paramInfo->plcTimeStampRaw,
  ...
  // Update time stamp
  if (paramInfo->timeBase == ADS_TIME_BASE_EPICS ||
      paramInfo->plcTimeStampRaw == 0) {
...
asynStatus adsAsynPortDriver::adsUpdateParameter(adsParamInfo *paramInfo,
  ...
  case ADST_INT8:
    ...
    case asynParamInt64:
      ret =
          setInteger64Param(paramInfo->paramIndex, (epicsInt64)(*ADST_INT8Var));
      break;
#endif
    case asynParamFloat64:
...
asynStatus adsAsynPortDriver::setAlarmParam(adsParamInfo *paramInfo, int alarm,
  ...
  // Alarm status or severity changed=>Do callbacks with old buffered data (if
  // nElemnts==0 then no data in record...)
  if (paramInfo->plcDataIsArray && paramInfo->arrayDataBuffer and
      paramInfo->arrayDataBufferSize > 0 && allowCallbackEpicsState) {
    switch (paramInfo->asynType) {
    case asynParamInt8Array:
      stat = doCallbacksInt8Array((epicsInt8 *)paramInfo->arrayDataBuffer,
                                  writeSize / sizeof(epicsInt8),
                                  paramInfo->paramIndex, paramInfo->asynAddr);
      break;
    case asynParamInt16Array:
      stat = doCallbacksInt16Array((epicsInt16 *)paramInfo->arrayDataBuffer,
                                   writeSize / sizeof(epicsInt16),
                                   paramInfo->paramIndex, paramInfo->asynAddr);
      break;
    case asynParamInt32Array:
      stat = doCallbacksInt32Array((epicsInt32 *)paramInfo->arrayDataBuffer,
    ...
    case asynParamFloat32Array:
      stat =
          doCallbacksFloat32Array((epicsFloat32 *)paramInfo->arrayDataBuffer,
                                  writeSize / sizeof(epicsFloat32),
                                  paramInfo->paramIndex, paramInfo->asynAddr);
      break;
    case asynParamFloat64Array:
      stat =
          doCallbacksFloat64Array((epicsFloat64 *)paramInfo->arrayDataBuffer,
                                  writeSize / sizeof(epicsFloat64),
                                  paramInfo->paramIndex, paramInfo->asynAddr);
      break;
    default:
 ...
 * Thread safe.
 */
asynStatus adsAsynPortDriver::setAlarmPortLock(uint16_t amsPort, int alarm,
                                               int severity) {
  asynStatus stat;
  lock();
...
/** Take adsLib lock.
 */
void adsAsynPortDriver::adsLock() { adsMutex.lock(); }

/** Release adsLib lock.
 */
void adsAsynPortDriver::adsUnlock() { adsMutex.unlock(); }
...
static void printHelp() {
  ...
  printf(" 2. AMS of plc                                 : "
         "\"192.168.88.44.1.1\"\n");
  printf(" 3. Default ams port                           : 851 for plc 1, 852 "
         "plc 2 ...\n");
  printf(" 4. Parameter table size (max parameters)      : 1000 example\n");
  printf(" 5. priority                                   : 0\n");
  printf(" 6. disable auto connect                       : 0 (autoconnect "
         "enabled)\n");
  printf(" 7. default sample time ms                     : 500 (check if "
         "variable changed each 500ms)\n");
  printf(" 8. max delay time ms (buffer time in plc)     : 1000 (if changed, "
         "send data atleast each 1000ms or faster if send buffer is full)\n");
  printf(" 9. ADS command timeout in ms                 : 1000 (timeout for "
         "adsLib commands)\n");
  printf(" 10. default time source (PLC=0,EPICS=1).      : 0 (PLC) NOTE: "
         "record TSE field need to be set to -2 for timestamp in asyn "
         "(field(TSE, -2))\n");
  printf("\n");
  printf(" Resulting adsAsynPortDriverConfigure() command: \n");
  printf(" adsAsynPortDriverConfigure(\"ADS_1\",\"192.168.88.44\",\"192.168.88."
         "44.1.1\",851,1000,0,0,50,100,1000,0)\n");
  printf("\n");
  printf("\n");
  printf(" NOTE: An ADS route needs to be added to the TwinCAT router of the "
         "controller/PLC:\n");
  printf("       1. \"TwinCAT->System->Routes->Static Routes\": Press \"Add\" "
         "button.\n");
  printf("       2. \"Route Name (Target)\": Enter name of EPICS machine.\n");
  printf("       3. \"AMSNetId\": Enter IP of EPICS machine. Add \".1.1\" in "
         "the end (x.x.x.x.1.1).\n");
  ...
  printf("       9. Ensure that the route was successfully added in the "
         "\"Static Routes\" list.\n");
  printf("\n");
 ...
 */
epicsShareFunc int adsAsynPortDriverConfigure(
    const char *portName, const char *ipaddr, const char *amsaddr,
    unsigned int amsport, unsigned int asynParamTableSize,
    unsigned int priority, int noAutoConnect, int defaultSampleTimeMS,
    int maxDelayTimeMS, int adsTimeoutMS, int defaultTimeSource) {
  ...
  if (defaultSampleTimeMS < 0) {
    printf("adsAsynPortDriverConfigure bad defaultSampleTimeMS: %dms. Standard "
           "value of 100ms will be used.\n",
           defaultSampleTimeMS);
    defaultSampleTimeMS = 100;
  }

  if (maxDelayTimeMS <= 0) {
    printf("adsAsynPortDriverConfigure bad maxDelayTimeMS: %dms. Standard "
           "value of 500ms will be used.\n",
           maxDelayTimeMS);
    maxDelayTimeMS = 500;
  }

  if (adsTimeoutMS <= 0) {
    printf("adsAsynPortDriverConfigure bad adsTimeoutMS: %dms. Standard value "
           "of 2000ms will be used.\n",
           adsTimeoutMS);
    adsTimeoutMS = 2000;
  }

  if (defaultTimeSource < 0 || defaultTimeSource >= ADS_TIME_BASE_MAX) {
    printf("adsAsynPortDriverConfigure bad default time source: %d. PLC time "
           "stamps will be used. Valid options are: PLC=%d and EPICS=%d.\n",
           defaultTimeSource, (int)ADS_TIME_BASE_PLC, (int)ADS_TIME_BASE_EPICS);
    defaultTimeSource = ADS_TIME_BASE_PLC;
  }

  adsAsynPortObj = new adsAsynPortDriver(
      portName, ipaddr, amsaddr, amsport, asynParamTableSize, priority,
      noAutoConnect == 0, defaultSampleTimeMS, maxDelayTimeMS, adsTimeoutMS,
      (ADSTIMESOURCE)defaultTimeSource);
  if (adsAsynPortObj) {
    asynUser *traceUser = adsAsynPortObj->getTraceAsynUser();
    if (!traceUser) {
      printf("adsAsynPortDriverConfigure: ERROR: Failed to retrieve asynUser "
             "for trace. \n");
      return (asynError);
    }
 ...
 */
static const iocshArg adsAsynPortDriverConfigureArg0 = {"port name",
                                                        iocshArgString};
static const iocshArg adsAsynPortDriverConfigureArg1 = {"ip-addr",
                                                        iocshArgString};
static const iocshArg adsAsynPortDriverConfigureArg2 = {"ams-addr",
                                                        iocshArgString};
static const iocshArg adsAsynPortDriverConfigureArg3 = {"default-ams-port",
                                                        iocshArgInt};
static const iocshArg adsAsynPortDriverConfigureArg4 = {"asyn param table size",
                                                        iocshArgInt};
static const iocshArg adsAsynPortDriverConfigureArg5 = {"priority",
                                                        iocshArgInt};
static const iocshArg adsAsynPortDriverConfigureArg6 = {"disable auto-connect",
                                                        iocshArgInt};
static const iocshArg adsAsynPortDriverConfigureArg7 = {
    "default sample time ms", iocshArgInt};
static const iocshArg adsAsynPortDriverConfigureArg8 = {"max delay time ms",
                                                        iocshArgInt};
...
static void adsAsynPortDriverConfigureCallFunc(const iocshArgBuf *args) {
  adsAsynPortDriverConfigure(args[0].sval, args[1].sval, args[2].sval,
...
static const iocshFuncDef adsSetLocalAddressFuncDef = {"adsSetLocalAddress", 1,
...
static const iocshFuncDef adsPollInfoFuncDef = {"adsPollInfo", 1,
                                                adsPollInfoArgs};
...
static void adsAsynPortDriverRegister(void) {
  iocshRegister(&adsAsynPortDriverConfigureFuncDef,
                adsAsynPortDriverConfigureCallFunc);
