...
class adsAsynPortDriver : public asynPortDriver
{
  ...
  private:
    ...
    asynStatus adsGetSymInfoByName(uint16_t amsPort, const char* varName, adsSymbolEntry* info);
    ...
    asynStatus adsReleaseSymbolicHandle(adsParamInfo* paramInfo, bool blockErrorMsg);
    asynStatus adsConnect();
    asynStatus adsDisconnect();
    asynStatus
    adsWriteParam(adsParamInfo* paramInfo, const void* binaryBuffer, uint32_t bytesToWrite);
    asynStatus adsReadParam(adsParamInfo* paramInfo);
    asynStatus adsReadParam(adsParamInfo* paramInfo, long* error, int updateAsynPar);
    asynStatus adsReadState(uint16_t* adsState);
    asynStatus adsReadStateLock(uint16_t amsport, uint16_t* adsState, bool blockErrorMsg);
    asynStatus
    adsReadStateLock(uint16_t amsport, uint16_t* adsState, bool blockErrorMsg, long* error);
    asynStatus adsReadState(uint16_t amsport, uint16_t* adsState, bool blockErrorMsg, long* error);
    ...
    int octetCmdHandleInputLine(const char* input_line, adsOctetOutputBufferType* buffer);
    int octetMotorHandleOneArg(const char* myarg_1, adsOctetOutputBufferType* buffer);
    int octetMotorHandleADRCmd(const char* arg, uint16_t adsport, adsOctetOutputBufferType* buffer);
