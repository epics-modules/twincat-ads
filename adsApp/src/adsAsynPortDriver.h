...
public:
  adsAsynPortDriver(const char *portName, const char *ipaddr,
                    ...
                    int paramTableSize, unsigned int priority, int autoConnect,
                    int defaultSampleTimeMS, int maxDelayTimeMS,
                    int adsTimeoutMS, ADSTIMESOURCE defaultTimeSource);
...
private:
  ...
  asynStatus adsDelDataCallback(adsParamInfo *paramInfo, bool blockErrorMsg);
  ...
  asynStatus adsGetSymInfoByName(uint16_t amsPort, const char *varName,
                                 adsSymbolEntry *info);
  asynStatus adsGetSymInfoByName(uint16_t amsPort, const char *varName,
                                 adsSymbolEntry *info, long *errorCode);
  asynStatus adsGetSymHandleByName(adsParamInfo *paramInfo);
  asynStatus adsGetSymHandleByName(adsParamInfo *paramInfo, bool blockErrorMsg);
  asynStatus adsReleaseSymbolicHandle(adsParamInfo *paramInfo);
  asynStatus adsReleaseSymbolicHandle(adsParamInfo *paramInfo,
                                      bool blockErrorMsg);
  asynStatus adsConnect();
  asynStatus adsDisconnect();
  asynStatus adsWriteParam(adsParamInfo *paramInfo, const void *binaryBuffer,
  ...
  asynStatus adsReadStateLock(uint16_t amsport, uint16_t *adsState,
                              bool blockErrorMsg);
  asynStatus adsReadStateLock(uint16_t amsport, uint16_t *adsState,
                              bool blockErrorMsg, long *error);
  asynStatus adsReadState(uint16_t amsport, uint16_t *adsState,
                          bool blockErrorMsg, long *error);
  asynStatus adsWriteState(uint16_t amsport, uint16_t adsState);

  asynStatus adsDelRoute(int force);
  asynStatus adsGenericArrayWrite(asynUser *pasynUser, long allowedType,
                                  const void *epicsDataBuffer,
                                  size_t nEpicsBufferBytes);
  asynStatus adsGenericArrayRead(asynUser *pasynUser, long allowedType,
                                 void *epicsDataBuffer,
                                 size_t nEpicsBufferBytes, size_t *nBytesRead);
  ...
  // Octet interface methods (ascii command parser through readoctet() and
  // writeoctet())
  int octetCMDreadIt(char *outbuf, size_t outlen);
  int octetCMDwriteIt(const char *inbuf, size_t inlen);
  ...
  int octetMotorHandleOneArg(const char *myarg_1,
                             adsOctetOutputBufferType *buffer);
  int octetMotorHandleADRCmd(const char *arg, uint16_t adsport,
                             adsOctetOutputBufferType *buffer);
  int octetAdsReadByName(uint16_t amsPort, const char *variableAddr,
                         adsOctetOutputBufferType *outBuffer);
  int octetAdsWriteByName(uint16_t amsPort, const char *variableAddr,
                          const char *asciiValueToWrite,
                          adsOctetOutputBufferType *outBuffer);
  int octetAdsReadByGroupOffset(uint16_t amsPort, adsSymbolEntry *info,
                                adsOctetOutputBufferType *outBuffer);
  int octetAdsWriteByGroupOffset(uint16_t amsPort, uint32_t group,
                                 uint32_t offset, uint16_t dataType,
                                 uint32_t dataSize,
                                 const char *asciiValueToWrite,
